...
with tabs[0]:
    ...
    pick = st.selectbox("shape_ph", keys2 + keys3)
...
with tabs[1]:
    keys2 = list(SHAPES_2D.keys())
    keys3 = list(SHAPES_3D.keys())
    pick = st.selectbox("shape_geo", keys2 + keys3)
...
with tabs[2]:
    keys2 = list(SHAPES_2D.keys())
    keys3 = list(SHAPES_3D.keys())
    pick = st.selectbox("shape_den", keys2 + keys3)
    ...
    _show(plot_diagram(dg[1], "H1 (full set)"))
...
with tabs[3]:
    keys2 = list(SHAPES_2D.keys())
    pick = st.selectbox("shape_cc", keys2)

    pts = SHAPES_2D[pick](n=int(n_points), noise=float(noise_2d), seed=int(seed) + 707)

    theta, birth, death = (None, None, None)
    if use_circ:
        theta, birth, death = circular_coordinates(pts, coeff=int(circ_coeff))
    _show(plot_points(pts, f"{pick} (colored by theta if enabled)", c=theta if theta is not None else None))
...
with tabs[4]:
    keys2 = list(SHAPES_2D.keys())
    keys3 = list(SHAPES_3D.keys())
    pick = st.selectbox("shape_map", keys2 + keys3)

    if pick in SHAPES_2D:
        pts = SHAPES_2D[pick](n=int(n_points), noise=float(noise_2d), seed=int(seed) + 808)
    else:
        pts = SHAPES_3D[pick](n=int(n_points), noise=float(noise_3d), seed=int(seed) + 909)
    ...
    st.write({"nodes": int(len(G["nodes"])), "edges": int(len(G["edges"]))})
    st.write({"spectral_dim": int(spec.shape[0])})
...
with tabs[5]:
    ...
    if start and not st.session_state.training:
        ...
        for i, p in enumerate(pcs):
            _maybe_stop()
            dg = dgms_only(p, maxdim=int(maxdim))
            dgms_euc.append(dg)
        ...
        tda_len = int(tda_feature_block(dgms_euc[0], grid_len=int(grid_len), topk=int(topk), k_levels=int(k_levels)).shape[0])
...
with tabs[6]:
    ...
    if pts is not None:
        ...
        if st.session_state.models is not None and st.session_state.fit_pack is not None:
            ...
            theta = None
            if fp["use_circ"] and pts.shape[1] == 2:
